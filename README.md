...
-   _그럼에도 Recoil_ : 와...... 엄청 편하다... 끙끙거리면서 context 쓸 때와 몇 번의 단계(action, store 어쩌구 저쩌구.. 여기에다가 saga까지 끼얹어지면 어이쿠!)가 펼쳐지는 redux 와 비교하면 정말 편하다. 상태관리 하려고 파일을 엄청 왔다갔다 해야하는 번거로움이 줄어드는 것 자체로도 이건 매우 큰 이점을 가지고 있다. redux를 쓰고 recoil로 넘어오라는 뜻이 이해가 됐다. 만약 모르고 왔다면 왜 이점인지 모르고 썼을 거 같다. redux를 기반으로 한 zustand도 궁금해졌는데, 이건 다음 기회에 recoil과 비교해서 꼭 써보고 싶다.
...
### 상태관리를 꼭 하나로만 해야하는 건 아니지!

-   Recoil을 쓰는 곳을 보다보면 Context API도 같이 쓰는 곳이 종종 보이던데 Recoil이 Context API를 기반으로 한 거라 가능한 일인가 싶다. 그래서 드는 생각은 상태관리에 있어서 꼭 하나만 써야 한다는 생각을 버리자는거다. Recoil과 Redux toolkit 을 같이 쓰는 건 응? 싶지만 저 두 개를 같이 썼다는 건 고려해야 할만한 상황이 올 수도 있다는 소리. 생각을 너무 틀에 가두지 말자.
